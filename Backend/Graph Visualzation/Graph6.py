...
total_schools = df.nunique().max()

df_total_schools = pd.DataFrame({"Total Number of Schools": [total_schools]})

styled_df6 = df_total_schools.style.set_properties(**{
    'border': '1px solid black', 'text-align': 'center', 'background-color': '#FADADD', 'color': '#333', 'font-size': '14px'
}).set_caption(f"📍 Total Number of Schools Across All Regions")\
.set_table_styles([
...
html_path = "styled_df6.html"
with open(html_path, "w", encoding="utf-8") as f:
    f.write(styled_df6.to_html())
